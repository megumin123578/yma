...
def create_sqlite_auth_schema(db_engine: Engine) -> None:
...
def ensure_sqlite_auth_state(db_engine: Engine) -> None:
    ...
    ensure_rival_groups_table(db_engine)
    ensure_smmstore_analytics_cache_table(db_engine)
    ensure_smmstore_scheduled_orders_table(db_engine)
    ensure_user_schedules_nullable_token(db_engine)
    ensure_user_credentials_token_name(db_engine)
