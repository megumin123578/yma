...
def fetch_inside(wid, log_fn=None):
    """Bước 1b: lấy data Inside kênh chính qua YouTube Analytics API.

    Chỉ chạy khi: config bật `inside_auto_fetch` + có thư viện Google +
    kênh chính có token OAuth. Thiếu/lỗi → log + skip, KHÔNG dừng
    pipeline. Đóng gói được vào exe.
    """
    try:
        from .config import load_config
        if not load_config().get("inside_auto_fetch", True):
            return
    except Exception:
        pass
    try:
        from . import inside_fetch
        from . import watchlist as wl_mod
    except Exception as e:
        _log(log_fn, f"  ! Inside fetch: không import được ({e})")
        return
    if not getattr(inside_fetch, "AVAILABLE", False):
        _log(log_fn, "  ! Inside fetch: thiếu thư viện Google API — bỏ qua.")
        return
    wl = wl_mod.load_watchlist(wid)
    if not wl:
        return
    sc = wl.self_channel
    if not sc or not getattr(sc, "channel_id", ""):
        return
    _log(log_fn, "Bước 1b: Lấy data Inside kênh chính "
                 "(YouTube Analytics API)...")
    try:
        res = inside_fetch.fetch_for_channel(
            sc.channel_id, log_fn=lambda m: _log(log_fn, m))
        if res.get("ok"):
            _log(log_fn, "  Inside: đã cập nhật data kênh chính.")
        else:
            _log(log_fn, f"  Inside: bỏ qua — {res.get('reason', '')}")
    except Exception as e:
        _log(log_fn, f"  ! Inside fetch lỗi (skip): {e}")
